    ...
    <div class="relative h-80">
        ...
        <div class="flex justify-end p-4">
            <button id="btn-unfollow" onclick="unfollowUser({{$user->id}})" class="@if (!$isFollowing)
                hidden
            @endif border cursor-pointer rounded-2xl p-2 w-30">Seguindo</button>
            <button id="btn-follow" onclick="followUser({{$user->id}})" class=" @if ($isFollowing)
                hidden
            @endif border cursor-pointer rounded-2xl p-2 bg-white text-black w-30 font-bold">Seguir</button>
        </div>
